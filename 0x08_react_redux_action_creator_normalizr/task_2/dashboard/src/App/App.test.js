...
describe('<App />', () => {
  ...
  it('should display CourseList when isLoggedIn is true', () => {
    const wrapper = shallow(<App />);
    wrapper.instance().logIn('test@test.com', 'password'); // Simulate login
  ...
  it('calls logOut function and displays alert when Ctrl+H is pressed', () => {
    ...
    const wrapper = shallow(<App />);
    wrapper.instance().logOut = mockLogOut; // Mock the logOut function
    wrapper.instance().componentDidMount(); // Attach the event listener
  ...
  // Test that markNotificationAsRead works as intended
  it('should remove a notification when markNotificationAsRead is called', () => {
    ...
    instance.markNotificationAsRead(2); // Mark notification with ID 2 as read
